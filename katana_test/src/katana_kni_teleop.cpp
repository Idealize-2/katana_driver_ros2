/*
 * katana_kni_teleop.cpp — ROS 2 / KNI low-level keyboard teleop
 *
 * Controls motors 0-5 directly via the KNI SDK, bypassing ros2_control.
 * Useful for raw hardware diagnostics: calibrates on startup, then moves
 * the arm to a "straight up" home position before entering the teleop loop.
 *
 * Usage:  ros2 run katana_test katana_kni_teleop [<ip>] [<port>]
 ...
 *
 * NOTE: This is the low-level KNI diagnostic tool.
 *       For normal teleop use "ros2 run katana_teleop katana_teleop_key"
 *       which operates through ros2_control.
 */
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::fprintf(stderr,
            "[kni_teleop] Cannot locate kni share dir: %s\n"
            "             Did you source install/setup.zsh ?\n", e.what());
        return 1;
    }

    std::printf("[kni_teleop] Connecting to %s:%d ...\n", ip, port);
    std::printf("[kni_teleop] Config: %s\n", cfg.c_str());
    ...
    try {
        ...
        std::printf("[kni_teleop] Connected.\n");
    } catch (const std::exception& e) {
        std::fprintf(stderr, "[kni_teleop] Connection failed: %s\n", e.what());
        return 1;
    }
    ...
    // ── Calibrate ────────────────────────────────────────────────────────────
    std::printf("[kni_teleop] Clearing fault flags...\n");
    try { g_katana->unBlock(); } catch (...) {}

    std::printf("[kni_teleop] Calibrating (arm moves to all joint limits) ...\n");
    try {
        g_katana->calibrate();
        g_katana->unBlock();
        std::printf("[kni_teleop] Calibration complete.\n");
    } catch (const std::exception& e) {
        std::fprintf(stderr, "[kni_teleop] Calibration failed: %s\n", e.what());
        cleanup();
        return 1;
    }
    ...
    for (int i = 0; i < arm_motors; ++i) {
        ...
        if (home_enc[i] != raw)
            std::printf("[kni_teleop]   motor%d home encoder = %d  (clamped from %d, limits [%d,%d])\n",
                        i, home_enc[i], raw, enc_min, enc_max);
        else
            std::printf("[kni_teleop]   motor%d home encoder = %d\n", i, home_enc[i]);
    }

    // ── Move to home ─────────────────────────────────────────────────────────
    std::printf("[kni_teleop] Moving to home (straight up) ...\n");
    try {
        g_katana->moveRobotToEnc(home_enc, /*wait=*/true, /*tol=*/100, /*timeout=*/30000);
        std::printf("[kni_teleop] Home position reached.\n");
    } catch (const std::exception& e) {
        std::fprintf(stderr, "[kni_teleop] Move to home failed: %s\n", e.what());
        cleanup();
        return 1;
    }
    ...
    std::printf("\n");
    std::printf("  +--------------------------------------------------+\n");
    std::printf("  |   Katana 400 Low-Level KNI Teleop (diagnostic)   |\n");
    std::printf("  +--------------------------------------------------+\n");
